...
__all__ = [
    'LocalModuleConfigWidget',
    'RemoteModuleConfigWidget',
    'ModuleConnectorsWidget',
    'ModuleOptionsWidget',
]
...
from qudi.core.config.validator import (
...
from qudi.tools.config_editor.custom_widgets import (
    CustomOptionsWidget,
    CustomConnectorsWidget,
)
...
class ModuleConnectorsWidget(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        layout.addWidget(
            self.custom_connectors_widget, len(self._connector_editors) + 2, 0, 1, 2
        )
...
class ModuleOptionsWidget(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        self._mandatory_names = (
            list() if mandatory_names is None else list(mandatory_names)
        )
        self._optional_names = (
            list() if optional_names is None else list(optional_names)
        )
        if set(self._mandatory_names).intersection(self._optional_names):
            raise ValueError(
                'ConfigOption names can not be both mandatory AND optional'
            )
        ...
        self.custom_options_widget = CustomOptionsWidget(
            forbidden_names=list(self._option_editors)
        )
        layout.addWidget(
            self.custom_options_widget, len(self._option_editors) + 2, 0, 1, 2
        )
...
class LocalModuleConfigWidget(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        config: Optional[
            Dict[str, Union[str, bool, Dict[str, str], Dict[str, Any]]]
        ] = None,
        parent: Optional[QtWidgets.QWidget] = None,
    ) -> None:
        super().__init__(parent=parent)
    ...
    def set_config(
        self,
        config: Union[
            None, Dict[str, Union[str, bool, Dict[str, str], Dict[str, Any]]]
        ],
    ) -> None:
        if config:
    ...
    def _get_connector_targets(
        ...
        for conn in connectors:
            targets = [
                ...
                if mod in valid_targets[conn.name]
            ]
            if conn.optional:
    ...
    def _validate_and_mark_config(self) -> None:
        try:
            ...
            print(
                f'Invalid local module config. Problematic fields: {list(err.relative_path)}'
            )
...
class RemoteModuleConfigWidget(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        self.native_name_lineedit.setToolTip(
            'The native module name as configured on the remote '
        ...
        self.remote_host_lineedit.setToolTip(
            'The IP address of the remote host. Can also be '
    ...
    def set_config(self, config: Union[None, Dict[str, Union[None, int, str]]]) -> None:
        if config:
            ...
            self.native_name_lineedit.setText(
                native_module_name if native_module_name else ''
            )
    ...
    def _validate_and_mark_config(self) -> None:
        try:
            ...
            print(
                f'Invalid remote module config. Problematic fields: {list(err.relative_path)}'
            )
