...
class QudiModules:
    ...
    def __init__(self):
        ...
        self._module_connectors_compatible_modules = {
            mod: self._modules_for_connectors(conn)
            for mod, conn in self._module_connectors.items()
        }
    ...
    def _modules_for_connector(self, connector: Connector) -> List[str]:
        ...
        bases = {
            mod: {c.__name__ for c in cls.mro()}
            for mod, cls in self._qudi_modules.items()
        }
    ...
    def module_connector_targets(self, module: str) -> Dict[str, List[str]]:
        return {
            k: v.copy()
            for k, v in self._module_connectors_compatible_modules[module].items()
        }
