...
class QudiMainGui(GuiBase):
    ...
    _automatic_status_var_dump = StatusVar(name="automatic_status_var_dump", default=0)
    _automatic_status_var_dump_interval = StatusVar(
        name="automatic_status_var_dump_interval", default=5
    )
    signal_update_automatic_status_var_checkstate = QtCore.Signal(int)
    ...
    def on_activate(self):
        ...
        if isinstance(version, str):
            ...
            self.mw.version_label.setText(
                ...
                "".format(
                    version[0], version[1], self._qudi_main.configuration.file_path
                )
            )
    ...
    def _connect_signals(self):
        get_signal_handler().sigRecordLogged.connect(
            self.handle_log_record, QtCore.Qt.QueuedConnection
        )
        qudi_main = self._qudi_main
        # Connect up the main windows actions
        self.mw.action_quit.triggered.connect(
            qudi_main.prompt_quit, QtCore.Qt.QueuedConnection
        )
        ...
        self.mw.action_open_configuration_editor.triggered.connect(
        ...
        qudi_main.module_manager.sigModuleAppDataChanged.connect(
            self.update_module_app_data
        )
        # Settings dialog
        self.mw.settings_dialog.accepted.connect(self.apply_settings)
        self.mw.settings_dialog.rejected.connect(self.keep_settings)
        self.error_dialog.disable_checkbox.clicked.connect(
            self._error_dialog_enabled_changed
        )
        # Modules list
        self.mw.module_widget.sigActivateModule.connect(
            qudi_main.module_manager.activate_module
        )
        self.mw.module_widget.sigReloadModule.connect(
            qudi_main.module_manager.reload_module
        )
    ...
    def reset_default_layout(self):
        ...
        self.mw.addDockWidget(
            QtCore.Qt.BottomDockWidgetArea, self.mw.threads_dockwidget
        )
    ...
    def stop_jupyter_widget(self):
        ...
        try:
            ...
            self.log.exception(
                "Exception while trying to shutdown qudi IPython client:"
            )
        try:
            self.mw.console_widget.kernel_manager.shutdown_kernel()
        except:
            self.log.exception(
                "Exception while trying to shutdown qudi IPython kernel:"
            )
        self._has_console = False
