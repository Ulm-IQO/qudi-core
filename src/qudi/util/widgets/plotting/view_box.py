...
class MouseTrackingMixin:
    ...
    def __init__(
        self, allow_tracking_outside_data: Optional[bool] = False, **kwargs
    ) -> None:
    ...
    def mouseDragEvent(self, ev: MouseDragEvent, axis: Optional[int] = None) -> None:
        if self.allow_tracking_outside_data or self.pointer_on_data(
            ev.buttonDownScenePos()
        ):
            start = self.mapToView(ev.buttonDownPos())
            current = self.mapToView(ev.pos())
            self.sigMouseDragged.emit(
                (start.x(), start.y()), (current.x(), current.y()), ev
            )
        if not ev.isAccepted():
    ...
    def pointer_on_data(self, scene_pos: QtCore.QPointF) -> bool:
        ...
        if scene is not None:
            for item in scene.items(scene_pos):
                if isinstance(
                    item, (PlotDataItem, ImageItem, PlotCurveItem, ScatterPlotItem)
                ):
                    return True
        return False
...
class DataSelectionMixin:
    ...
    def __init__(
        self,
        selection_bounds: Optional[
            Sequence[Tuple[Union[None, float], Union[None, float]]]
        ] = None,
        ...
        **kwargs,
        ...
        self._selection_bounds = (
            None if selection_bounds is None else list(selection_bounds)
        )
        ...
        if (xy_region_min_size_percentile is not None) and (
            xy_region_min_size_percentile > 0
        ):
    ...
    def _update_xy_region_min_size(self, viewbox, new_range, changed) -> None:
        min_size = [
            self._xy_region_min_size_percentile * abs(rang[1] - rang[0])
            for rang in new_range
        ]
    ...
    def set_selection_bounds(
        self, bounds: Union[None, List[Union[None, Tuple[float, float]]]]
    ) -> None:
    ...
    def _add_region_selection(
        ...
        if mode == self.SelectionMode.XY:
            ...
            item = item_type(
                ...
                and self._xy_region_selection_handles,
    ...
    def move_region_selection(
        self, span: Tuple[Tuple[float, float], Tuple[float, float]], index: int
    ) -> None:
        self._move_region_selection(span, index)
        self._emit_region_change()

    def _move_region_selection(
        self, span: Tuple[Tuple[float, float], Tuple[float, float]], index: int
    ) -> None:
    ...
    def _move_marker_selection(self, position: Tuple[float, float], index: int) -> None:
        ...
        if isinstance(item, InfiniteLine):
            item.set_position(
                position[0 if item.orientation == QtCore.Qt.Vertical else 1]
            )
    ...
    def marker_selection(
        self,
    ) -> Dict[SelectionMode, List[Union[float, Tuple[float, float]]]]:
        return {
    ...
    def region_selection(self) -> Dict[SelectionMode, List[tuple]]:
        return {
            self.SelectionMode.X: [
                r.area
                for r in self.__regions
                if isinstance(r, LinearRegion) and r.orientation == QtCore.Qt.Vertical
            ],
            self.SelectionMode.Y: [
                r.area
                for r in self.__regions
                if isinstance(r, LinearRegion) and r.orientation == QtCore.Qt.Horizontal
            ],
            self.SelectionMode.XY: [
                r.area for r in self.__regions if isinstance(r, Rectangle)
            ],
        }
    ...
    def _apply_selection_bounds(self) -> None:
        ...
        for m in self.__markers:
            if isinstance(m, InfiniteLine):
                m.set_bounds(
                    x_bounds if m.orientation == QtCore.Qt.Vertical else y_bounds
                )
        ...
        for r in self.__regions:
            if isinstance(r, LinearRegion):
                r.set_bounds(
                    x_bounds if r.orientation == QtCore.Qt.Vertical else y_bounds
                )
...
class RubberbandZoomSelectionViewBox(
    RubberbandZoomMixin, DataSelectionMixin, MouseTrackingMixin, ViewBox
):
    pass
