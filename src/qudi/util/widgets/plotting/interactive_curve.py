...
class PlotEditorWidget(QtWidgets.QWidget):
    ...
    def limits(self) -> Tuple[Tuple[float, float], Tuple[float, float]]:
        x_min, x_max = sorted(
            [self.x_lower_limit_spinBox.value(), self.x_upper_limit_spinBox.value()]
        )
        y_min, y_max = sorted(
...
class PlotSelectorWidget(QtWidgets.QWidget):
    ...
    def selection(self) -> Dict[str, bool]:
        return {
            name: selector.isChecked()
            for name, (_, selector) in self._selectors.items()
        }
    ...
    def add_selector(
        ...
        if name in self._selectors:
            raise ValueError(
                f'Selector with name "{name}" already present in plot selector'
            )
...
class InteractiveCurvesWidget(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.set_rubberband_zoom_selection_mode = (
            self._plot_widget.set_rubberband_zoom_selection_mode
        )
    ...
    def toggle_cursor_position(self, enable: bool) -> None:
        ...
        if is_enabled and not enable:
            self._plot_widget.sigMouseMoved.disconnect(
                self._position_label.update_position
            )
        elif not is_enabled and enable:
            self._plot_widget.sigMouseMoved.connect(
                self._position_label.update_position
            )
    ...
    def __labels_changed(
        self, x: Optional[str] = None, y: Optional[str] = None
    ) -> None:
        if x is y is None:
            return
    ...
    def __plot_widget_limits_changed(
        self, _, limits: Tuple[Tuple[float, float], Tuple[float, float]]
    ) -> None:
