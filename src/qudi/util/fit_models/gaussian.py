...
class DoubleGaussian(FitModelBase):
    ...
    def _model_function(
        ...
        return offset + multiple_gaussian(
...
class GaussianLinear(FitModelBase):
    ...
    def _model_function(x, offset, slope, center, sigma, amplitude):
        ...
        return (
            offset
            + x0 * slope
            + multiple_gaussian(x, (center,), (sigma,), (amplitude,))
        )
...
class Gaussian2D(FitModelBase):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.set_param_hint("offset", value=0, min=-np.inf, max=np.inf)
        self.set_param_hint("amplitude", value=0.0, min=-np.inf, max=np.inf)
        self.set_param_hint("center_x", value=0.0, min=-np.inf, max=np.inf)
        self.set_param_hint("center_y", value=0.0, min=-np.inf, max=np.inf)
        self.set_param_hint("sigma_x", value=0, min=0, max=np.inf)
        self.set_param_hint("sigma_y", value=0, min=0, max=np.inf)
        self.set_param_hint("theta", value=0.0, min=-np.pi, max=np.pi)

    @staticmethod
    def _model_function(
        x, offset, amplitude, center_x, center_y, sigma_x, sigma_y, theta
    ):
        try:
            a = np.cos(-theta) ** 2 / (2 * sigma_x**2) + np.sin(-theta) ** 2 / (
                2 * sigma_y**2
            )
            b = np.sin(2 * -theta) / (4 * sigma_y**2) - np.sin(2 * -theta) / (
                4 * sigma_x**2
            )
    ...
    def estimate_peak(self, data, x):
        ...
        estimate["center_x"].set(
            value=center_x,
            min=np.min(x[0]) - x_range / 2,
            max=np.max(x[0]) + x_range / 2,
        )
        estimate["center_y"].set(
            value=center_y,
            min=np.min(x[1]) - y_range / 2,
            max=np.max(x[1]) + y_range / 2,
        )
        estimate["sigma_x"].set(
            value=sigma_x, min=x_range / (x[0].shape[0] - 1), max=x_range
        )
        estimate["sigma_y"].set(
            value=sigma_y, min=y_range / (x[0].shape[1] - 1), max=y_range
        )
