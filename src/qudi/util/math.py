...
def compute_ft(x_val, y_val, zeropad_num=0, window='none', base_corr=True, psd=False):
    """
    Compute the Discrete Fourier Transform (DFT) or the Power Spectral Density (PSD) 
    ...
    zeropad_num : int, optional
        Zero-padding (adding zeros to the end of the array). If `zeropad_num >= 0`, 
        the size of the array is extended by adding zeros to the end of `y_val` 
        before performing the Fourier Transform (FT). The resulting array will have 
        the length `(len(y_val)/2)*(zeropad_num+1)`. Zero-padding does not add more 
        information to the DFT; it only interpolates between the DFT values. Set 
        `zeropad_num=1` to obtain output arrays of the same size as the input arrays. 
        Default is `zeropad_num=0`.
    window : str, optional
        The window function to be applied to the y-values before calculating the FT. 
        If not specified, no window function is applied.
    base_corr : bool, optional
        Whether baseline correction should be performed before calculating the FT. 
        Default is `False`.
    psd : bool, optional
        Whether to compute the Power Spectral Density (PSD) instead of the Discrete 
        Fourier Transform (DFT). The PSD is the FT of the absolute square of the 
        y-values. Default is `False`.

    Returns
    -------
    tuple of numpy.array
        A tuple containing the DFT or PSD results:
        - dft_x : numpy.array
            The x-values of the DFT/PSD.
        - dft_y : numpy.array
            The y-values of the DFT/PSD.
        
        Note that the length of the return arrays depends on `zeropad_num`:
        `len(dft_x) = len(dft_y) = (len(y_val)/2)*(zeropad_num+1)`.

    Notes
    -----
    - The return values of the FT have only half of the entries compared to the used signal input 
      if `zeropad_num=0`.
    - In general, a window function should be applied to the y-data before calculating the FT to 
      reduce spectral leakage. The Hann window is often a good choice, e.g., `window='hann'`.
    - Keep in mind the relation to the Fourier transform space:
        T = delta_t * N_samples
      where `delta_t` is the distance between time points and `N_samples` is the number of points 
      in the time domain. Consequently, the sample rate is:
        f_samplerate = T / N_samples
      The FT returns values from 0 to `f_samplerate`, or equivalently `-f_samplerate/2` to 
      `f_samplerate/2`.
    - Difference between PSD and DFT:
      The Power Spectral Density (PSD) describes how the power of your signal is distributed 
      over frequency, while the DFT shows the spectral content of your signal, i.e., the 
      amplitude and phase of harmonics in your signal.
    """
